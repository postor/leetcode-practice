...
var findContentChildren = function (g, s) {
  let s1 = s.sort((a, b) => a - b), g1 = g.sort((a, b) => a - b), count = 0
  //step 1 greedy
  let i = g1.length - 1, j = s1.length - 1
  while (i >= 0 && j >= 0) {
    // if (g1[i] == s1[j]) {
    if (g1[i] <= s1[j]) {
      count++
  ...
  // function r(childStartIndex = 0, leftCookies = []) {
  //   let key = `${childStartIndex}|${leftCookies.join(',')}`
  //   if (cache[key] !== undefined) return cache[key]
  //   let maxCount = 0
  //   for (let i = childStartIndex; i < g1.length; i++) {
  //     let [canContentI, afterContentCookies] = findMinContentCookie(g1[i], leftCookies)
  //     if (!canContentI) break
  //     maxCount = Math.max(1 + r(i + 1, afterContentCookies))
  //   }
  //   cache[key] = maxCount
  //   return maxCount
  // }

  // function findMinContentCookie(content, cookies = []) {
  //   if (!cookies.length || cookies[cookies.length - 1] < content) {
  //     return [false]
  //   }
  //   let index = findMinContentIndex(0, cookies.length - 1)
  //   let t = cookies.concat()
  //   t.splice(index, 1)
  //   return [true, t]

  //   function findMinContentIndex(left, right) {
  //     if (left == right) return left
  //     let mid = Math.floor((left + right) / 2)

  //     if (cookies[mid] < content) {
  //       return findMinContentIndex(mid + 1, right)
  //     }
  //     // if (cookies[mid] > content) {
  //     return findMinContentIndex(left, mid)
  //   }
  // }
