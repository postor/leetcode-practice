...
var reversePairs = function (nums) {
  ...
  let count = 0, sorted = []
  doubles.forEach((x, i) => {
    let c = sortedCountLargerThan(x)
    if (c > 0) count += c
    sortedPush(nums[i])
  })
  return count

  function sortedPush(x) {
    let i = sortedFindIndex(x)
    sorted.splice(i, 0, x)
    // console.log(sorted.join(','))
  }

  function sortedCountLargerThan(x) {
    let i = sortedFindIndex(x)
    let j=i
    for(;j<sorted.length;j++){
      if(x!=sorted[j]) break
    }
    return sorted.length - j
  }

  function sortedFindIndex(x) {
