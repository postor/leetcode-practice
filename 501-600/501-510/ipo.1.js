...
var findMaximizedCapital = function (k, W, Profits, Capital) {
  let indexes = Profits.map((x, i) => i)
  indexes.sort((a, b) => Capital[a] - Capital[b]) // sort by profit

  let money = W, left = k, covered = new SortedArray([]), i = 0
  while (left) {
    if (i < indexes.length) {
      let j = indexes[i]
      while (money >= Capital[j]) {
        covered.insert(Profits[j])
        i++
        j = indexes[i]
      }
    }
    let arr = covered.array
    if (!arr.length) return money
...
console.log(findMaximizedCapital(111
  , 12
