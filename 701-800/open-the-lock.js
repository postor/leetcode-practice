...
var openLock = function (deadends, target) {
  ...
  setDpStr(target, -3)
  while (true) {
    let hasNext = false, tmpDp = dp.map(x => x.map(x => x.map(x => x.map(x => x))))
    for (let i0 = 0; i0 < 10; i0++) {
      for (let i1 = 0; i1 < 10; i1++) {
        for (let i2 = 0; i2 < 10; i2++) {
          for (let i3 = 0; i3 < 10; i3++) {
            let pass = [i0, i1, i2, i3]
            let val = getDp(pass)
            // deadend or tried
            if (val == -2 || val >= 0) continue
            let min = Number.MAX_SAFE_INTEGER
            for (let i = 0; i < 4; i++) {
              let tarr = [pass.concat(), pass.concat()]
              // move ith wheel up
              tarr[0][i] = (pass[i] + 1) % 10
              // move down
              tarr[1][i] = (pass[i] + 9) % 10
              for (let tpass of tarr) {
                let tpassV = getDp(tpass)
                if (tpassV < 0) continue
                min = Math.min(min, tpassV)
              }
            }
            if (min == Number.MAX_SAFE_INTEGER) continue
            if (val == -3) return min + 1
            setDp(pass, min + 1, tmpDp)
            hasNext = true
          }
        }
      }
  ...
  function setDp([i0, i1, i2, i3] = [], v = 0, dp1 = dp) {
    dp1[i0][i1][i2][i3] = v
  }
